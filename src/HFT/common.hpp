...
struct vote
{
  ...
  std::string voteTime; // Unix-time of the rating
...
{
public:
  corpus(std::string trdata_path, std::string vadata_path, std::string tedata_path, int max)
  {
    ...
    TR_V = new std::vector<vote*>();
    TE_V = new std::vector<vote*>();
    VA_V = new std::vector<vote*>();
    vote* v = new vote();
  ...
  ~corpus()
  {
    for (std::vector<vote*>::iterator it = TR_V->begin(); it != TR_V->end(); it++)
      delete *it;
    delete TR_V;
    for (std::vector<vote*>::iterator it = VA_V->begin(); it != VA_V->end(); it++)
      delete *it;
    delete VA_V;
    for (std::vector<vote*>::iterator it = TE_V->begin(); it != TE_V->end(); it++)
      delete *it;
    delete TE_V;
  }
