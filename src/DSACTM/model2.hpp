...
public:
    DSACTM(char* trdata_path, char* vadata_path, char* tedata_path,
        ...
        this->alpha       = alpha;
        this->inner_niters= inner_niters;
        ...
        for (vector<Vote*>::iterator it = corp->TR_V->begin(); 
                it != corp->TR_V->end(); it++) {
#ifdef CD_DEBUG
            /*cout << "User: " << corp->ruser_ids[(*it)->user] << endl;
            cout << "Item: " << corp->ritem_ids[(*it)->item] << endl;
            cout << "Rating: " << (*it)->value << endl;
            cout << "Word: ";
            for (vector<int>::iterator it1=(*it)->words.begin();
                    it1!=(*it)->words.end(); it1++)
                cout << corp->rword_ids[*it1] << " ";
            cout << endl;
            utils::pause();*/
#endif
...
#pragma omp parallel for
    ...
    void gdBatchLearning() {
        ...
        evalRmseError(train_rmse, valid_rmse, test_rmse);
        printf("Before training, Train RMSE=%.6f, ", train_rmse);
        printf("Valid RMSE=%.6f, Test RMSE=%.6f!\n",
                valid_rmse, test_rmse);
        
        printf("Start iterative conditional learning...\n");
        timeval start_t, end_t;
        obj_old = 1e5;
        cur_valid = 1e5;
        best_valid = cur_valid;
                ...
                for (vector<Vote*>::iterator it=train_votes.begin();
                    ...
                    for (int k=0; k<K; k++) {
                        tmp_val1[k] = psai_u*(gamma_user[user][k]
                                      -theta_user[user][k]);
                        tmp_val2[k] = psai_i*(gamma_item[item][k]
                                      -theta_item[item][k]);
                        // compute gradient of user latent factor
                        gtheta_user[k] = -theta_item[item][k]*res + tmp_val1[k];
                        // compute gradient of item latent factor
...
//#endif
            ...
            for (int t=0; t<inner_niters; t++) {
...
#ifdef GBD_DEBUG
                        //cout << "exp_wordtopic_sum:" << exp_wordtopic_sum <<endl;
#endif
                        //#pragma omp parallel for
                        for (int k=0; k<K; k++) 
                            exp_doctopic_sum[k] += exp_wordtopic[k]
                                                 / exp_wordtopic_sum;
                    }
#ifdef GBD_DEBUG
                    /*cout << "Words num:" << (*it)->words.size() << endl;
                    cout << "exp_doctopic_sum: ";
                    for (int k=0; k<K; k++) {
                        cout << exp_doctopic_sum[k] << " ";
                    }
                    cout << endl;*/
#endif
                    //#pragma omp parallel for
                    for (int k=0; k<K; k++)
                        grad_bt[k]+= exp_doctopic_sum[k]
                                   * (1-exp_topic[k]/exp_topic_sum);
#ifdef GBD_DEBUG
                    /*cout << "Gradient of background topic: ";
                ...
                //#pragma omp parallel for
#ifdef GBD_DEBUG
                /*cout << "Before, Background Topic: ";
                for (int kk=0; kk<K; kk++)
                    cout << background_topic[kk] << " ";
                cout << endl;
                cout << "Gradient of background topic: ";
                for (int kk=0; kk<K; kk++)
                    cout << grad_bt[kk] << " ";
...
#ifdef GBD_DEBUG
            utils::toc(start_t, end_t);
            cout << "After inner iterations, Background Topic: ";
            for (int k=0; k<K; k++)
                cout << background_topic[k] << " " << endl;
            cout << endl;
            utils::pause();
#endif
            ...
            for (int u=0; u<n_users; u++) {
                ...
                for (int t=0; t<inner_niters; t++) {
                    ...
                    for (vector<Vote*>::iterator it=train_votes_puser[u].begin();
                        ...
                        for (int k=0; k<K; k++) {
                            exp_topic_u[u][k] = exp_gu[u][k]*exp_gi[item][k]
                                              * exp_bt[k];
        ...
        while(!converged && cur_iter < niters) {
            ...
            for (int u=0; u<n_users; u++) {
                ...
                for (vector<Vote*>::iterator it=train_votes_puser[u].begin();
                    ...
                    arma::colvec r(theta_item[item], K, true, true);
                    M = M + r*r.t();
                    Q = Q + r*res;
                }
                M = M+psai_u*arma::eye<arma::mat>(K, K);
                cout << "M:" << M << endl;
                cout << "Q:" << Q.t() << endl;
                Q = arma::inv(M)*Q;
                cout << "Q:" << Q.t() << endl;
                ...
                for (int k=0; k<K; k++)
                    theta_user[u][k]=Q(k);
#ifdef CD_DEBUG
                printf("\rFinished user num: %d...", u+1);
                fflush(stdout);
#endif
            }
#ifdef CD_DEBUG
            ...
            utils::pause();
            evalRmseError(train_rmse, valid_rmse, test_rmse);
            printf("After training users, Train RMSE=%.6f, ", train_rmse);
            printf("Valid RMSE=%.6f, Test RMSE=%.6f!\n",
                    valid_rmse, test_rmse);
#ifdef CD_DEBUG
            utils::toc(start_t, end_t);
            ...
            evalRmseError(train_rmse, valid_rmse, test_rmse);
            printf("After training user bias, Train RMSE=%.6f, ", train_rmse);
            printf("Valid RMSE=%.6f, Test RMSE=%.6f!\n",
                    valid_rmse, test_rmse);
#ifdef CD_DEBUG
            utils::toc(start_t, end_t);
            ...
            for (int i=0; i<n_items; i++) {
                ...
                for (vector<Vote*>::iterator it=train_votes_pitem[i].begin();
                    ...
                    int user = (*it)->user;
                    double res = (*it)->value-predictionWithoutFactorProd(*it);
                    arma::colvec r(theta_user[user], K, true, true);
                    M = M + r*r.t();
                    Q = Q + r*res;
                }
                M = M + psai_i*arma::eye<arma::mat>(K, K);
                Q = arma::inv(M)*Q;
                for (int k=0; k<K; k++)
                    theta_item[i][k]=Q(k);
            }
            evalRmseError(train_rmse, valid_rmse, test_rmse);
            printf("After training items, Train RMSE=%.6f, ", train_rmse);
            printf("Valid RMSE=%.6f, Test RMSE=%.6f!\n",
                    valid_rmse, test_rmse);
#ifdef CD_DEBUG
            utils::toc(start_t, end_t);
            ...
            evalRmseError(train_rmse, valid_rmse, test_rmse);
            printf("After training item bias, Train RMSE=%.6f, ", train_rmse);
            printf("Valid RMSE=%.6f, Test RMSE=%.6f!\n",
                    valid_rmse, test_rmse);
#ifdef CD_DEBUG
            utils::toc(start_t, end_t);
            ...
            *mu = inter_mu/(train_votes.size()+sigma_a);
            ...
            evalRmseError(train_rmse, valid_rmse, test_rmse);
            printf("After training whole bias, Train RMSE=%.6f, ", train_rmse);
            printf("Valid RMSE=%.6f, Test RMSE=%.6f!\n",
                    valid_rmse, test_rmse);
#ifdef CD_DEBUG
            utils::toc(start_t, end_t);
            ...
            for (int i=0; i<n_items; i++) {
                for (int t=0; t<inner_niters; t++) {
                    ...
                    for (vector<Vote*>::iterator it=train_votes_pitem[i].begin();
                        ...
                        exp_topic_sum_i[i] = 0.0;
                        for (int k=0; k<K; k++) {
                            exp_topic_i[i][k]=exp_gu[user][k]*exp_gi[i][k]*exp_bt[k];
                            exp_topic_sum_i[i] += exp_topic[k];
                        }
                        memset(exp_doctopic_sum_i[i], 0.0, sizeof(double)*K);
                        for (vector<int>::iterator it1=(*it)->words.begin();
